...
public class ZAsyncTask_yihe extends AsyncTask<Bill, Void, Bill> {
    private LinkedHashMap<String, String> map_errorCode_message_distinguish;

    {
        map_errorCode_message_distinguish = new LinkedHashMap<>();
        map_errorCode_message_distinguish.put("001", "正常返回结果");
        map_errorCode_message_distinguish.put("002", "拒绝反复查验同一发票");
        map_errorCode_message_distinguish.put("003", "发票查验请求太频繁，请稍后再试");
        map_errorCode_message_distinguish.put("004", "超过服务器最大请求数，请稍后访问");
        map_errorCode_message_distinguish.put("005", "请求不合法");
        map_errorCode_message_distinguish.put("006", "发票信息不一致");
        map_errorCode_message_distinguish.put("009", "无此发票");
        map_errorCode_message_distinguish.put("010", "网络超时，请重试");
        map_errorCode_message_distinguish.put("011", "内部错误");
        map_errorCode_message_distinguish.put("099", "用户key失效");
        map_errorCode_message_distinguish.put("097", "输入参数格式不正确或空");
    }
...
//                "data": {
//                    "xfmc": "深圳市旋极天下贸易有限公司",
//                    "xfsh": "440300326687017",
//                    "xfdz": "深圳市宝安区福永街道骏泰商务中心1120号0755-33159545",
//                    "xfzh": "中国建设银行股份有限公司深圳宝安支行0701364000002030331",
//                    "gfmc": "深圳市宏伟清洗材料科技有限公司",
//                    "gfsh": "440300695577374",
//                    "gfdz": "深圳市龙岗区坪地街道吉祥一路13号，0755-89944928",
//                    "gfzh": "工商银行深圳市龙岗支行0701003539800016260",
//                    "jym": "35625554013218879926",
//                    "jqbm": "661525975298",
//                    "spxx": "商品1=规格=单位=数量=单价=金额=税率=税额≡商品2=规格=单位=数量=单价=金额=税率=税额",
//                    "je": "3632.12",
//                    "zfbz": "N",	//作废标志
//                    "bz": "备注"
//                   }
//        }
    ...
    //合合发票识别api
    private final String url_bill_recognize = "https://imgs-sandbox.intsig" +
            ".net/icr/recognize_vat_invoice";
    //合合发票验真所需json格式
    private final String url_bill_distinguish = "https://imgs-sandbox.intsig.net/icr/verify_vat";

    private CallBack callBack;

    public ZAsyncTask_yihe(CallBack callBack) {
    ...
    protected Bill doInBackground(Bill... bills) {
        ...
        switch (bill.getFunc()) {
            case Bill.FUNC_USER_BILL_RECOGNIZE:
                ...
                if (w_bitmap < 1200) {
                    float percnet_w = 1200f / w_bitmap;
                    bitmap = B.getBitmapByPercent(bitmap, percnet_w);
                }
                if (h_bitmap < 1200) {
                    float percnet_h = 1200f / h_bitmap;
                    bitmap = B.getBitmapByPercent(bitmap, percnet_h);
                }
                if (bytes.length < (5 * 1024 * 1024)) {//限制图片大小为5Mb
                    ZHttpTools.Request request = new ZHttpTools.Request(B.bitmapToBytes(bitmap),
                            ZHttpTools.Request.content_type_bit, ZHttpTools.Request.way_post, url);
                ...
                S.s("abcd_bitmap", "图片大小[" + bytes.length + "] 换算:" + bytes.length / 1024f /
                        1024f + "M");
                break;
...
//    }
                ...
                try {
                    ...
                    try {
                        ZHttpTools.Request request = new ZHttpTools.Request(json.getBytes
                                ("UTF-8"), ZHttpTools.Request.content_type_json, ZHttpTools
                                .Request.way_post, url);
                        ZHttpTools.Response response = ZHttpTools.request(request);
                        switch (response.code) {
                            case 200:
    ...
    private void parseJson_recognize(Bill bill, String json) {
        ...
        try {
            ...
            S.s("发票识别返回码: error_code:[" + error_code + "]" + getMap_errorCode_message_distinguish
                    ().get(String.valueOf(error_code)));
        } catch (JSONException e) {
            e.printStackTrace();
            S.e(e);
    ...
    public void parseJson_distinguish(Bill bill, String json) {
        ...
        try {
            ...
            S.s("发票验真返回码: error_no:[" + error_no + "]" + getMap_errorCode_message_distinguish()
                    .get(String.valueOf(error_no)));
        } catch (JSONException e) {
            e.printStackTrace();
            S.e(e);
        ...
        try {
            ...
            for (int i = 0; i < keys_payer_and_seller.length; i++) {
                try {
                    bill.putKeyValue(keys_payer_and_seller[i] + "_payer", jsonObject_payer
                            .getString(keys_payer_and_seller[i]));
                } catch (JSONException e) {
                    e.printStackTrace();
                    S.e(e);
                }
            ...
            for (int i = 0; i < keys_payer_and_seller.length; i++) {
                try {
                    bill.putKeyValue(keys_payer_and_seller[i] + "_seller", jsonObject_seller
                            .getString(keys_payer_and_seller[i]));
                } catch (JSONException e) {
                    e.printStackTrace();
                    S.e(e);
